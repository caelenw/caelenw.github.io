/* every one second go through and 
move the class of highlighted threw the gallery 
*/
/*Preferred solution for highlighting set interval 
setInterval(() => {
    let currentSection = document.querySelector(".items section.highlighted");
   
    //highlight the first item
    if(currentSection == null ){
        document.querySelector(".items section").classList.add("highlighted"); //first section within items 
        return;
    }
...
setInterval(()=>{
    ...
    //add highlighting to the current selction with the current count
